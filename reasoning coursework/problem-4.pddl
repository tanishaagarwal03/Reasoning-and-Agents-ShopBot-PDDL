(define (problem supermarketextension3)
  (:domain supermarketextension3)
  ...
  (:init
    ...
    (adjacent scale c1) (adjacent scale c4) 
    ...
    ;; Define the initial bot positions and states
    (bot-is-at shopbot1 c20)
    (bot-is-at shopbot2 c19)
    (empty-hands shopbot1)
    (empty-hands shopbot2)
    (is-occupied c20)
    (is-occupied c19)
    ...
    ;; Define initial vasket position and availability
    ...
    (needs-weighing cabbage shopbot2)
    (needs-weighing potato shopbot1)

    ;; Define items that have to be checked out and by which bot
    (needs-to-be-checked-out potato shopbot1)
    (needs-to-be-checked-out ketchup shopbot1)
    (needs-to-be-checked-out toothpaste shopbot1)
    (needs-to-be-checked-out pizza shopbot1)
    (needs-to-be-checked-out cabbage shopbot2)
    (needs-to-be-checked-out bread shopbot2)
    (needs-to-be-checked-out shampoo shopbot2)
    (needs-to-be-checked-out ice_lolly shopbot2)

    ;; Define the credit balances and item prices
    (= (credit-balance shopbot1) 15)
    (= (credit-balance shopbot2) 11)
    (= (price ice_lolly) 4)
    (= (price pizza) 6)
    (= (price cabbage) 3)
    (= (price potato) 4)
  ...
  (:goal
    (and
      (checked-out potato shopbot1) 
      (checked-out ketchup shopbot1) 
      (checked-out toothpaste shopbot1) 
      (checked-out pizza shopbot1)
      (checked-out cabbage shopbot2) 
      (checked-out bread shopbot2) 
      (checked-out shampoo shopbot2) 
      (checked-out ice_lolly shopbot2)
    )
  )
)
